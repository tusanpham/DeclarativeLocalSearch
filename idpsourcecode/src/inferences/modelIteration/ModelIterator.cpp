...
class SolverTerminationIt: public TerminateMonitor {
private:
	MinisatID::ModelIterationTask* solver;
public:
	SolverTerminationIt(MinisatID::ModelIterationTask* solver)
			: solver(solver) {
	}
...
MXResult ModelIterator::calculateMonitor() {
	auto terminator = std::make_shared<SolverTerminationIt>(_mx);
